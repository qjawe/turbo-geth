...
func BenchmarkTxn_Get_ro(b *testing.B) {
	...
	err = env.View(func(txn *Txn) (err error) {
		...
		defer b.StopTimer()
		for i := 0; i < b.N; i++ {
			v := ps[rand.Intn(len(ps))]
			_, err := txn.Get(dbi, v)
			if IsNotFound(err) {
				continue
			}
			if err != nil {
				b.Fatalf("error getting data: %v, %x", err, v)
			}
		}
...
func populateBenchmarkDB(env *Env, dbi DBI, rc *randSourceCursor) ([][]byte, error) {
	...
	err := env.SetGeometry(-1, -1, int(1*datasize.TB), int(1*datasize.GB), -1, 4096)
	if err != nil {
		return nil, err
	}
...
func openBenchDBI(b *testing.B, env *Env) DBI {
	var dbi DBI
	err := env.Update(func(txn *Txn) (err error) {
		dbi, err = txn.OpenDBI("benchmark", Create, nil, nil)
		return err
	})
	if err != nil {
