...
func (opts MdbxOpts) Open() (KV, error) {
	...
	_ = env.SetDebug(mdbx.LogLvlNotice, mdbx.DbgDoNotChange, mdbx.LoggerDoNotChange) // temporary disable error, because it works if call it 1 time, but returns error if call it twice in same process (what often happening in tests)
	...
	err = env.SetOption(mdbx.OptSpillMinDenominator, 2)
	if err != nil {
		return nil, err
	}
