...
func (opts MdbxOpts) Open() (KV, error) {
	...
	res := env.SetDebug(mdbx.LogLvlDoNotChange, mdbx.DbgDoNotChange, mdbx.LoggerDoNotChange) // temporary disable error, because it works if call it 1 time, but returns error if call it twice in same process (what often happening in tests)
	fmt.Printf("dbgres: %d\n", res)
