		...
		if account != nil && account.Root != trie.EmptyRoot {
			...
			if err1 != nil || subTries.Hashes[0] != account.Root {
				...
				for key, entry := range sm {
					...
					dbValue, _ := stateDb.Get(dbutils.CurrentStateBucketOld2, cKey[:])
					value := bytes.TrimLeft(entry.Value[:], "\x00")
					if !bytes.Equal(dbValue, value) {
						fmt.Printf("Key: %x, value: %x, dbValue: %x\n", key, value, dbValue)
						if err := stateDb.Put(dbutils.CurrentStateBucketOld2, cKey[:], value); err != nil {
							fmt.Printf("%v\n", err)
						}
					}
				}
				...
				copy(cKey[:], addrHash[:])
				binary.BigEndian.PutUint64(cKey[common.HashLength:], account.Incarnation)
				if err := stateDb.Walk(dbutils.CurrentStateBucketOld2, cKey[:], 8*(common.HashLength+common.IncarnationLength), func(k, v []byte) (bool, error) {
					var kh common.Hash
					copy(kh[:], k[common.HashLength+common.IncarnationLength:])
					if _, ok := sm[kh]; !ok {
						fmt.Printf("Key: %x, dbValue: %x\n", kh, v)
						if err := stateDb.Delete(dbutils.CurrentStateBucketOld2, k, nil); err != nil {
							fmt.Printf("%v\n", err)
						}
					}
					return true, nil
