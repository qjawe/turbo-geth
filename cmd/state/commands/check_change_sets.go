...
func init() {
	...
	withBlock(checkBlocksCmd)
	withChaindata(checkBlocksCmd)
	checkBlocksCmd.Flags().StringVar(&historyfile, "historyfile", "", "path to the file where the changesets and history are expected to be. If omitted, the same as --chaindata")
	rootCmd.AddCommand(checkBlocksCmd)
}
...
var checkBlocksCmd = &cobra.Command{
	...
	Short: "Re-executes historical transactions in read-only mode and checks that their outputs match the database ChangeSets",
	RunE: func(cmd *cobra.Command, args []string) error {
		return stateless.CheckBlocks(block, chaindata, historyfile)
	},
}
