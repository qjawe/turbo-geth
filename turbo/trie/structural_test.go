...
func Test2(t *testing.T) {
	...
	for _, key := range keys {
		...
		if curr.Len() > 0 {
			fmt.Printf("send: %x\n", succ.Bytes())
			groups, branches, err = GenStructStep2(func(_ []byte) bool { return false },
				curr.Bytes(), succ.Bytes(), hb,
				func(keyHex []byte, set uint16, branchesSet uint16, hashes []byte, rootHash []byte) error {
	...
	succ.Reset()
	// Produce the key which is specially modified version of `curr` (only different in the last nibble)
	if groups, _, err = GenStructStep2(func(_ []byte) bool { return false }, curr.Bytes(), succ.Bytes(), hb, func(keyHex []byte, set uint16, branchesSet uint16, hashes []byte, rootHash []byte) error {
		fmt.Printf("collect: %x, %b\n", keyHex, branchesSet)
		return nil
