...
var (
	...
	benchRootFunds  = uint256.NewInt().Lsh(u256.Num1, 100)
)
...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	gspec := Genesis{
		...
		Alloc:  GenesisAlloc{benchRootAddr: {Balance: benchRootFunds.ToBig()}},
	}
	...
	b.ReportAllocs()
	b.ResetTimer()
	if i, err := chainman.InsertChain(context.Background(), chain); err != nil {
...
func makeChainForBench(db ethdb.Database, full bool, count uint64) {
	...
	for n := uint64(0); n < count; n++ {
		...
		if full || n == 0 {
			...
			rawdb.WriteBodyFromNetwork(ctx, db, hash, n, block.Body())
