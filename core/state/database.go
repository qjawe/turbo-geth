...
func (tds *TrieDbState) UnwindTo(blockNr uint64) error {
	...
	for key, value := range storageMap {
		...
		if len(value) > 0 {
			m[keyHash] = value
			if err := tds.db.Put(dbutils.CurrentStateBucketOld2, []byte(key)[:common.HashLength+common.IncarnationLength+common.HashLength], value); err != nil {
				return err
			}
		} else {
			m[keyHash] = nil
			if err := tds.db.Delete(dbutils.CurrentStateBucketOld2, []byte(key)[:common.HashLength+common.IncarnationLength+common.HashLength], nil); err != nil {
				return err
			}
		}
	}
...
func (tds *TrieDbState) ReadAccountStorage(address common.Address, incarnation uint64, key *common.Hash) ([]byte, error) {
	...
	if !ok {
		...
		enc, err = tds.db.Get(dbutils.CurrentStateBucketOld2, dbutils.GenerateCompositeStorageKey(addrHash, incarnation, seckey))
		if err != nil {
			enc = nil
		}
	}
