...
func TestPruneStorageOfSelfDestructedAccounts(t *testing.T) {
	...
	putStorage := func(k string, v string) {
		require.NoError(db.Put(dbutils.CurrentStateBucketOld2, storageKey(k), common.Hex2Bytes(v)))
	}
	...
	v, err := db.Get(dbutils.CurrentStateBucketOld2, storageKey(k1))
	require.True(errors.Is(err, ethdb.ErrKeyNotFound))
	assert.Nil(v)

	v, err = db.Get(dbutils.CurrentStateBucketOld2, storageKey(k2))
	require.NoError(err)
